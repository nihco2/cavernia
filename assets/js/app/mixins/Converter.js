  ...
  var Converter = Ember.Mixin.create({
    convert: function(latitude, longitude) {
      ...
      return new Ember.RSVP.Promise(function(resolve, reject) {
        var r = Math.sqrt(Math.pow(latitude - 700000.0, 2) + Math.pow(longitude - 12655612.05, 2)),
          theta = Math.atan((latitude - 700000.0) / (12655612.05 - longitude)),
          lambda = 0.0523598776 + theta / 0.725607765,
          isomlat = -(1 / 0.725607765) * Math.log(Math.abs(r / 11754255.426)),
          phi = self.computeLatitude(isomlat, 0.08248325676, 0.00000000001),
          coords = {
